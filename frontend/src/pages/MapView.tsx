...
export default function MapView() {
  ...
  return (
    <div className="container mx-auto px-4 py-10">
      ...
      {loading ? (
        <div className="h-[70vh] w-full rounded-xl border flex items-center justify-center bg-muted/20">
          <div className="animate-spin md:w-8 md:h-8 border-4 border-primary border-t-transparent rounded-full"></div>
        </div>
      ) : (
        <SpeciesMap species={filtered} className="h-[70vh] w-full rounded-xl border shadow-sm relative z-0" />
      )}
    </div>
  );
}